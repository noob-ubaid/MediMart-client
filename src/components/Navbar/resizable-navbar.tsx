    ...
    >
      {items.map((item, idx) => (
        <NavLink
          ...
          onClick={onItemClick}
          className="relative px-4 py-2 font-primary font-medium text-lg text-white"
          key={`link-${idx}`}
          to={item.link}
        >
          {hovered === idx && (
            <motion.div
              layoutId="hovered"
              className="absolute inset-0 h-full w-full rounded-full bg-gray-900"
            />
          )}
...
export const NavbarButton = ({
  ...
  const baseStyles =
    "px-4 py-2 rounded-md bg-white text-black font-medium relative cursor-pointer transition duration-200 inline-block text-center";
